...
function Trendingmovies() {
    ...
    const fetchdata = async () => {
      try {
        const response = await fetch(
          `https://imdb146.p.rapidapi.com/v1/find/?query=Avengers`,
          {
            headers: {
  ...
  return (
    ...
    <h1 className='ml-20  font-bold text-2xl relative top-12'>Avengers</h1>
    <div  className="lg:grid-cols-5 mt-10 md:grid-cols-3 grid-cols-2 grid lg:ml-0 md:ml-10 ml-7 h-[60vh] p-3 gap-4 ">
      ...
      {tilteposter.map((item, index) => (
        ...
        <img   className="lg:h-[50vh] md:h-[50vh] h-[30vh] mt-8 hover:scale-105 transition-all drop-shadow-2xl rounded-xl" key={index} src={item.titlePosterImageModel?.url} alt="" /> 
        </Link>
      ))}
